...
def draw_N(ox,oy,r,n):  #ox,oy는 원점좌표, r은 반지름, n은 n각형
    ...
    total_angle_sum = (n-2)*pi #N각형에서 내각의 총합
    a=ox+r
    b =oy
    for i in range(n):
        #기준점(a,b)에서 삼각함수로 다음 점을 계산하여 서로 잇는다.
        mypaper.create_line(a,b,ox+r*cos((i+1)*(pi-total_angle_sum/n)),
        oy+r*sin((i+1)*(pi-total_angle_sum/n)),width=3)
 
        #a, b를 갱신
        a=ox+r*cos((i+1)*(pi-total_angle_sum/n))
        b=oy+r*sin((i+1)*(pi-total_angle_sum/n))
 
draw_N(300,300,200,7) # ox,oy,r,n순. 마지막 숫자가 n각형
